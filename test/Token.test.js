...
beforeEach(async () => {
  ...
  describe('failure', async() =>{
    ...
    it('rejects insufficient balances', async() =>{
      ...
      await token.transfer(receiver, invalidAmount,
        { from: deployer}).should.be.rejectedWith(EVM_REVERT)

      // Attempt transfer tokens, when you have none
      invalidAmount = tokens(10) // recipient has no tokens
      await token.transfer(deployer, invalidAmount,
        { from: receiver}).should.be.rejectedWith(EVM_REVERT)
        })

      // Attempt sending tokens to an invalid address
  ...
  describe('failure', async() =>{
    ...
    it('rejects insufficient balances', async() =>{
      ...
      const invalidAmount = tokens(10000000) // 100 million - greater than total supply
      await token.transferFrom(deployer, receiver, invalidAmount,
        { from: exchange}).should.be.rejectedWith(EVM_REVERT)
      })
      // Attempt sending tokens to an invalid address
      it('rejects invalid recipients', async () => {
      await token.transferFrom(deployer, 0x0, amount,
        { from: exchange }).should.be.rejected
    })
  })
})
